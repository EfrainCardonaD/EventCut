...
## Verificacion CORS (Vercel + ngrok)
- Asegura `CORS_ALLOWED_ORIGINS` con origen exacto, por ejemplo `https://event-cut.vercel.app`.
- Si usas `TrustedHostMiddleware`, incluye el host real (`*.ngrok-free.dev` en dev).

Preflight `OPTIONS` esperado: `200` con `Access-Control-Allow-Origin` y `Access-Control-Allow-Credentials`.

```powershell
$headers = @{
  Origin = "https://event-cut.vercel.app"
  "Access-Control-Request-Method" = "GET"
  "Access-Control-Request-Headers" = "authorization,content-type"
}
Invoke-WebRequest -Uri "https://TU-DOMINIO.ngrok-free.dev/api/v1/categories" -Method Options -Headers $headers
```

Nota ngrok (plan gratuito): si aparece una pagina HTML de advertencia en clientes SPA, agrega header
`ngrok-skip-browser-warning: true` en el cliente HTTP (fetch/Axios) solo para desarrollo.
