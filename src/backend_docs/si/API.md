...
```json
{
  ...
  "data": {
    ...
    "details": {},
    "trace_id": "b3d7f6c0f9bc40f0a8fbcf0a1cf4f8df"
  }
}
```

Nota: `DELETE /api/v1/events/{event_id}` mantiene `204 No Content` (sin body).
...
Nota: solo retorna eventos activos; el campo interno `is_active` no se expone en la API.
...
Nota: eventos inactivos responden `404`.
...
- `503`: auth-service no disponible para validacion remota (fail-closed)

Codigos de `data.code` mas frecuentes:
- `TOKEN_INVALID`
- `TOKEN_EXPIRED`
- `FORBIDDEN`
- `EVENT_NOT_FOUND`
